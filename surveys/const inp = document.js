const response = {
  ...
  "I give permission to Monarch Insurance Agency LLC and it’s affiliates to access and/or create my application for health insurance on the Federally Facilitated Marketplace (FFM) based on the information I am providing below.":
    "",
...
const Response = async () => {
  try {
    const res = await fetch(
      `https://script.google.com/macros/library/d/1_sAAMdf0ztt7d_cjpy7ORn5T2vGmFOvrfYq6u53qMFCbFEjTcabOMbQT/42`
    );
...
async function fetchDataAndSetFields(contactId) {
  ...
  try {
    ...
    for (const item in data) {
      if (data.hasOwnProperty(item)) {
        try {
          const findData = getInput(item);
          if (!findData) {
            console.error("get error:", item);
            continue;
          }

          const type = findData.type;
          const signatureBtn = findData.classList.contains("signature-button");
          let clearButton;

          if (type) {
            findData.value = data[item] || "";
            findData.dispatchEvent(new Event("input"));

            if (signatureBtn) {
              findData.style.backgroundImage = data[item]?.url;
              clearButton = findData.nextElementSibling;
            ...
            if (signatureBtn) {
              findData.style.backgroundImage = `url('${imageUrl}')`;
              clearButton = findData.nextElementSibling;
              if (clearButton?.classList.contains("clear-button")) {
                clearButton.style.display = "none";
              }
            } else {
              appendImage(imageUrl, findData);
            }
          }
        } catch (err) {
...
// Assuming response is available globally or in the relevant scope
function autofillForm() {
  for (const attribute in response) {
    if (response.hasOwnProperty(attribute)) {
      const fieldValue = response[attribute];
